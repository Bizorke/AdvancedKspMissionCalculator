...
function Booster(engine, numbEngines, numbTanks, dumpIntoBooster, dumpMultiplier, hasStageRecoveryChute){
	this.engine = engine;
	...
	this.overhead  //Stage recovery chutes, decouplers, and fuel ducts.
		= (dumpIntoBooster == null 
		? (engine.radius == 0.625 ? 15 : (engine.radius == 1.25 ? 50 : (engine.radius == 2.5 ? 400 : (800))))//stack decoupler
		: 50 + 25);//Fuel pipes + radial decoupler
	
	this.numbSrChutes = 0;
	
	if(hasStageRecoveryChute){
		var parachuteResult = calculateParachutes(this.getDryMass(), planetData.kerbin);
		if(parachuteResult.status == "success"){
			this.numbSrChutes = parachuteResult.numbChutes;
			this.overhead += this.numbSrChutes * 100; //100kg each.
		}
		else{
			this.numbSrChutes = -1;
		}
	...
	this.hasStageRecoveryChute = hasStageRecoveryChute;
}

Booster.prototype.getDryMass = function(){
	return this.overhead
		+ this.engine.mass * this.numbEngines
