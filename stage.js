...
function Stage(stageType, engine, numbEnginesPerBooster, numbTanksPerBooster, numbRadialBoosterPairs, cargoMass){
	...
	if(engine != null){
		...
		this.mainBooster = new Booster(engine, numbEnginesPerBooster, numbTanksPerBooster, null, 0);
		for(var i = 0; i < numbRadialBoosterPairs; i++){
			this.detachableBoosters.push(new Booster(engine, numbEnginesPerBooster, numbTanksPerBooster, (this.detachableBoosters.length < 2 ? this.mainBooster : this.detachableBoosters[this.detachableBoosters.length - 2]), (i == 0 ? 0.5 : 1)));
			this.detachableBoosters.push(new Booster(engine, numbEnginesPerBooster, numbTanksPerBooster, (this.detachableBoosters.length < 2 ? this.mainBooster : this.detachableBoosters[this.detachableBoosters.length - 2]), 1));
		}
	}
	else{
...
Stage.prototype.clone = function(){
	...
	var stage = new Stage(this.type, this.engine || this.removedEngine, this.numbEnginesPerBooster, this.numbTanksPerBooster, numbBoosterPairs, this.cargoMass);
